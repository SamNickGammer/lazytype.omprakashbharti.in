APP_NAME ?= LazyType Dev
BUNDLE_ID ?= in.omprakashbharti.lazytype.dev
BUILD_DIR = build
APP_BUNDLE = $(BUILD_DIR)/$(APP_NAME).app
CODESIGN_IDENTITY ?= LazyType Dev
...
# which LazyType they are running when both are installed side by side.
ifeq ($(APP_NAME),LazyType Dev)
...
ifeq ($(ARCH),universal)
	...
	@codesign --force --options runtime --sign "$(CODESIGN_IDENTITY)" --entitlements LazyType.entitlements "$(APP_BUNDLE)"
