---
name: lazytype-release
description: Prepare and publish LazyType app releases. Use when the user asks to release LazyType, prepare a new version, bump the LazyType version, write or update LazyType changelog entries, validate a LazyType semver release, create a vX.Y.Z tag, or publish a signed LazyType DMG through the repository GitHub Actions release workflow.
---
...
Use this skill to prepare a LazyType release from the local repository. LazyType releases are semver-tag driven: pushing a tag like `v0.3.1` triggers `.github/workflows/release.yml`, which stamps the app bundle, extracts the matching `CHANGELOG.md` section, builds/signs/notarizes the DMG, and creates the GitHub Release.
...
5. Validate locally before commit/tag:
   ...
   .agents/skills/lazytype-release/scripts/lazytype-release-check.sh <version>
   git diff --check
   make clean
...
8. After GitHub Actions finishes, verify:
   ...
   - `LazyType.dmg` is attached.
   - Release body starts with the matching `CHANGELOG.md` section.
   - A previous app version detects the update and shows What’s New.

## Helper Script

Run `.agents/skills/lazytype-release/scripts/lazytype-release-check.sh <version>` from the LazyType repo root to check release preconditions. It validates semver shape, required files, changelog extraction, workflow trigger basics, and tag availability.
