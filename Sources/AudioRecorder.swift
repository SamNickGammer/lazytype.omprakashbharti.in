...
final class AudioRecorder: NSObject, ObservableObject, AVCaptureAudioDataOutputSampleBufferDelegate {
    ...
    private let sessionQueue = DispatchQueue(label: "in.omprakashbharti.lazytype.capture.session")
    private let sampleBufferQueue = DispatchQueue(label: "in.omprakashbharti.lazytype.capture.samples")
