...
import Carbon
import os.log
private let recordingLog = OSLog(subsystem: "in.omprakashbharti.lazytype", category: "Recording")
...
enum AppBuild {
    static var isDevBundle: Bool {
        (Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String) == "LazyType Dev"
    }
}
...
final class AppState: ObservableObject, @unchecked Sendable {
    ...
    /// URL of the flag file written while LazyType is actively recording.
    ///
    ...
    ///
    /// Path: `~/Library/Application Support/LazyType/is-recording`
    /// (or `LazyType Dev/is-recording` when running the dev bundle).
    static func recordingStateFlagURL() -> URL {
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        let appName = Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String ?? "LazyType"
    ...
    private static let recordingStateFlagQueue = DispatchQueue(
        label: "in.omprakashbharti.lazytype.recording-state-flag"
    )
    ...
    func hasScreenCapturePermission() -> Bool {
        ...
        // of the process: once it returns false it keeps returning false even
        // after the user grants permission, until the app is relaunched. That
        // makes the setup screen appear "stuck" on Grant Access right after the
        // grant. To detect the grant live we fall back to probing window titles,
        // which macOS only exposes to apps that hold Screen Recording access.
        if CGPreflightScreenCaptureAccess() { return true }
        return canReadOtherAppWindowTitles()
    }

    /// Live Screen Recording probe that does not cache. Returns true if any
    /// other app's normal window exposes a non-empty title, which is only
    /// possible when this app holds Screen Recording permission.
    private func canReadOtherAppWindowTitles() -> Bool {
        ...
        for window in windows {
            guard let pid = window[kCGWindowOwnerPID as String] as? Int, pid != ownPID else { continue }
    ...
    private func beginCriticalDictationActivity() {
        ...
        ProcessInfo.processInfo.disableAutomaticTermination("LazyType dictation in progress")
        automaticTerminationDisabled = true
    }

    private func endCriticalDictationActivity() {
        guard automaticTerminationDisabled else { return }
        ProcessInfo.processInfo.enableAutomaticTermination("LazyType dictation in progress")
