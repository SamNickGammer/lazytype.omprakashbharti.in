...
struct SetupView: View {
    ...
    private let lazytypeRepoURL = URL(string: "https://github.com/SamNickGammer/lazytype.omprakashbharti.in")!
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: NSApplication.didBecomeActiveNotification)) { _ in
            // Re-measure every permission when the user switches back from
            // System Settings, so a freshly granted permission flips to
            // "Granted" immediately instead of waiting on a poll tick.
    ...
    var welcomeStep: some View {
        VStack(spacing: 16) {
            ...
            VStack(spacing: 10) {
                HStack(spacing: 8) {
                    AsyncImage(url: URL(string: "https://github.com/SamNickGammer.png?size=64")) { phase in
                        switch phase {
                    ...
                    Button {
                        openURL(lazytypeRepoURL)
                    } label: {
                        Text("SamNickGammer/lazytype.omprakashbharti.in")
            ...
            Group {
                ...
                case .recording:
                    VStack(spacing: 20) {
                        ZStack {
                            Circle()
                                .fill(Color.brand.opacity(0.65))
                                .frame(width: 100, height: 100)

                            Circle()
                                .stroke(Color.brand.opacity(0.8), lineWidth: 3)
                                .frame(width: 100, height: 100)
                                .shadow(color: .brand.opacity(0.5), radius: 10)
    ...
    var stepIndicator: some View {
        HStack(spacing: 8) {
            ForEach(totalSteps, id: \.rawValue) { step in
                Circle()
                    .fill(step == currentStep ? Color.brand : Color.gray.opacity(0.3))
    ...
    /// Re-check all three system permissions in one shot. Used when the app
    /// regains focus so newly granted access is reflected right away.
...
class GitHubMetadataCache: ObservableObject {
    ...
    private let repoAPIURL = URL(string: "https://api.github.com/repos/SamNickGammer/lazytype.omprakashbharti.in")!
    ...
    func fetchIfNeeded() async {
        ...
        do {
            ...
            if count > 0 {
                ...
                let stargazersURL = URL(string: "https://api.github.com/repos/SamNickGammer/lazytype.omprakashbharti.in/stargazers?per_page=\(perPage)&page=\(lastPage)")!
            ...
            let contributorsURL = URL(string: "https://api.github.com/repos/SamNickGammer/lazytype.omprakashbharti.in/contributors?per_page=15")!
            do {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ForEach(0..<3, id: \.self) { index in
                Circle()
                    .fill(Color.brand.opacity(activeDot == index ? 1.0 : 0.3))
