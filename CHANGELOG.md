...
All notable changes to LazyType are documented here.
...
LazyType is now considered feature-complete and stable enough for a 1.0 release.
...
- Transcription Language setting for choosing the language LazyType listens for during dictation.
- Recording state flag file for external tools that need to know when LazyType is actively recording.
- Distinct LazyType Dev app and menu bar icons so development builds are easier to tell apart from release builds.
...
- Fixed cases where LazyType could silently fall back when the selected microphone was unavailable.
...
- Direct shortcuts from LazyType to the right macOS permission settings.
...
- LazyType now warns you if alert sounds may be hard to hear because system audio is muted or very low.
- Update prompts now show the version, release date, and release notes more clearly.
- LazyType now uses proper version numbers for updates instead of internal build names.
